...
public:
    ...
    void start()
    {
        ...
        try
        {
            ...
            pqxx::result res{ txn.exec(
                "SELECT id, type, channel_id, message_text, next_run_time, interval_seconds, interval_str "
            ...
            for (const auto& row : res)
            {
                ...
                job.next_run_time = parse_db_time(row["next_run_time"].as<std::string>());

                long long seconds = row["interval_seconds"].is_null() ? 0 : row["interval_seconds"].as<long long>();
                job.interval = std::chrono::seconds(seconds);
                job.interval_str = row["interval_str"].is_null() ? "" : row["interval_str"].as<std::string>();

                jobs.insert(job);
            }
            std::cout << "[Scheduler] Successfully loaded " << jobs.size() << " job(s) from the database.\n";
        }
        catch (const std::exception& e)
        {
            std::cerr << "[Scheduler Error] Failed to load jobs from DB on start: " << e.what() << '\n';
        }
    ...
    )
    {
        ...
        {
            std::cerr << "[Scheduler Error] Failed to save once job to DB: " << e.what() << '\n';
            throw;
        }
        ...
        cv_.notify_one();
        std::cout << "[Scheduler] Scheduled one-off message ID " << new_id << " for " << time_str << '\n';
    }
        ...
        while (running)
        {
            ...
            else
            {
                ...
                if (soonest_job.next_run_time <= now)
                {
                    ...
                    lock.unlock(); // unlock job queue mutex so that others can schedule right away

                    try {
                        execute_job(soonest_job);
                    } catch (const std::exception& e) {
                        std::cerr << "[Scheduler Error] Loop task execution failed: " << e.what() << '\n';
                    }
                }
                else
                {
                    cv_.wait_until(lock, soonest_job.next_run_time, [this, soonest_job]{
                        if (!running) return true;
                        if (jobs.empty()) return true;
                        // wake up if soonest job changed to something sooner
                        return jobs.begin()->next_run_time < soonest_job.next_run_time;
                    });
                }
            }
        }
